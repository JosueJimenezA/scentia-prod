...
# ---------------------------------------------------------------------------
# RUTAS RELATIVAS RESPECTO A LA UBICACIÓN DEL SCRIPT
# ---------------------------------------------------------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

SEMILLA_FILE = os.path.join(BASE_DIR, "perfume_catalog_links.csv")
CHECKPOINT_FILE = os.path.join(BASE_DIR, "fragrantica_data_from_scraper.csv")
LOG_ERRORES_FILE = os.path.join(BASE_DIR, "failed_urls.log")
LOG_SESIONES_FILE = os.path.join(BASE_DIR, "scraping_sessions.log")
...
async def scrape_fragrantica_deep_raw(browser, url, semaphore):
    async with semaphore:
        ...
        try:
            ...
            # 2. Scroll rápido y dirigido (Remplaza human_scroll de 20s por 1s)
            ...
            if await reviews_trigger.count() > 0:
                try:
                    ...
                    pass  # Continuar si la sección de reseñas no responde en tiempo
...
def registrar_log_sesion(inicio_ts, total_pendientes, exitosos, fallidos, estado):
    ...
    # Se crea el archivo automáticamente con modo "a" (append) si no existe
...
async def main():
    ...
    # Verificación obligatoria de SEMILLA_FILE
    if not os.path.exists(SEMILLA_FILE):
        print(f"❌ Error crítico: El archivo semilla '{SEMILLA_FILE}' no existe.")
        return
    ...
    try:
        ...
        urls_objetivo = df_semilla["perfume_url"].dropna().tolist()
    except Exception as e:
        print(f"❌ Error al leer el archivo semilla '{SEMILLA_FILE}': {e}")
        return

    # Verificación opcional de CHECKPOINT_FILE (se creará sobre la marcha al guardar datos)
    urls_procesadas = set()
    if os.path.exists(CHECKPOINT_FILE):
        try:
            ...
            print(f"⚠️ No se pudo leer el checkpoint existente: {e}")
    ...
    async with async_playwright() as p:
        ...
        try:
            # Procesamiento en lotes concurrentes respetando el semáforo
            for index, url in enumerate(urls_pendientes, start=1):
                ...
                if data:
                    ...
                    # Si no existe CHECKPOINT_FILE, to_csv lo creará automáticamente con la cabecera
                ...
                # Pausa leve entre peticiones para evitar bloqueos por IP
